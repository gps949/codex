...
//!
//! A toy `1+1?` with empty tools and "Reply with one short token" is not a Codex
//! turn. Interactive sessions start the 5h window with the model's real
//! instructions plus the default CLI tool harness. Warmup reuses that payload
//! without creating a Session or calling activate/lease.
...
pub(crate) fn is_warmup_tool_call(item: &ResponseItem) -> bool {
    matches!(
        item,
...
fn warmup_tool_specs() -> Arc<[ToolSpec]> {
    Arc::from([
