...
- `accountPool/use` — experimental; activate a specific account profile (`profileId`, optionally `force` to clear a quota cooldown) or, when `profileId` is omitted, re-enter automatic scheduling using `[account_pool].rotation_strategy` (`fill_first` or `earliest_reset`). Setting `force` without `profileId` also allows that one user-requested automatic selection to probe a quota-cooled profile whose observed rate-limit reset is already due. With `earliest_reset`, idle 5h windows (`0%` used, countdown not yet started) are preferred over already-started future windows so real traffic can start their reset clocks earlier. With `fill_first`, equal-priority candidates also prefer an unstarted 5h window. Separately, `[account_pool].window_warmup` (default true) periodically sends a tiny identity-preserving generating request to standby profiles whose primary window is still at `0%` or unknown, then refreshes usage via GET; when the active account is at/above `preemptive_switch_percent` and standbys still need warming, the loop runs as often as every 5 minutes. Failed warmups back off for 15 minutes per profile and surface as `windowWarmup` on `accountPool/read` / picker rows (`5h warmed`, `warmup retry in H:MM`, etc.). Set `window_warmup = false` to disable; tune the normal cadence with `window_warmup_interval_minutes` (default 30, minimum 5).
...
- `account/rateLimits/read` — the Codex bucket uses a bounded active-account heading such as `Work · 2/3 ready`. Numeric windows and other model buckets retain their original values. Readiness means scheduler eligibility, not a verified quota balance. Unattributed streaming quota notifications are suppressed for pool-enabled mobile clients: an old in-flight request must not overwrite a newly selected account's snapshot. Reopening or refreshing Status fetches the authenticated snapshot; desktop streaming notifications are unchanged.
- `turn/start` — `/status` and `/account` refresh observed account quotas and return up to four accounts per page, with the current account first. Pool windows are labeled primary/secondary because the pool protocol does not carry their durations. `/account list <page>` reads another page; `/account show "<label-or-id>"` shows reset and observation times. Failed refreshes retain cached observations; unknown quota is explicit.
- Mobile controls: `/account use "<label-or-id>"` selects an eligible account, `/account auto` selects using the current rotation strategy, and `/account strategy [fill-first|earliest-reset]` reads or saves the strategy. Names must match exactly and ambiguous names fail. Selection affects subsequent requests, does not permanently pin an account, and does not bypass disabled or cooldown state. Strategy changes apply to the next automatic selection; use `/account auto` to select immediately. `/account help` lists commands. Invalid arguments produce an error reply instead of silently displaying the account list.
- These synthetic replies go only to the requesting connection and are not added to model history or persisted rollouts. They are transient and cannot be restored by history reload. A running turn still rejects chat commands to avoid replacing the real turn in older clients; the native Status panel remains available. Status captions are reconstructed on account reads. Local workspace account captions remain available when the optional backend banner service is unavailable. iOS foreground/reconnect rendering requires real-client verification.
